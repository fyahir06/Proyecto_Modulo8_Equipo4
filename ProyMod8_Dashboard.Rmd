...
poblacion_clean <- poblacion_raw %>%
  clean_names()
num_columnas <- ncol(poblacion_clean) 
umbral_na   <- 0.5 * num_columnas
poblacion_clean <- poblacion_clean %>%
  filter(rowSums(is.na(.)) <= umbral_na)
write_csv(poblacion_clean, "poblacion_limpia.csv")
...
# (subconjunto opcional, NO altera datos_TD)
datos_muestra <- datos_TD  # sin filtros
